...
export default function ProfilePage() {
    ...
    const fetchProfileData = async () => {
        ...
        try {
            ...
            // Initialize editableData for the form
            setEditableData({
                ...
                // Note: files are not directly set in editableData, but handled separately
                // The 'files' array in profileData contains server filenames, not File objects
            });
            ...
            <div className={`transition-all duration-300 ${sidebarOpen ? "ml-64" : "ml-0"}`}>
                ...
                >
                    <section className="bg-white rounded-3xl shadow-lg p-8 flex flex-col gap-8">
                        <div className="flex flex-col md:flex-row items-center justify-center gap-6">
                            <img
                                src={imagePreviewUrl || "/src/assets/default_avatar.png"} // Use the new state for image source
                                alt="Profile"
                            ...
                            <div className="text-center md:text-left">
                                ...
                                {/* File Input (Hidden) */}
                                {isEditing && ( // Only show file input and button when in editing mode
                                    <>
                                        <input
                                            type="file"
                                            id="file"
                                            name="file"
                                            ref={fileInputRef}
                                            onChange={handleProfileFileChange} // Use dedicated handler
                                            className="hidden"
                                            accept="image/jpeg,image/png,image/jpg,image/webp"
                                        />
                                        <button
                                            onClick={() => fileInputRef.current.click()}
                                            className="mt-2 px-4 py-2 bg-[#891839] text-white rounded hover:bg-[#6c122e]"
                                        >
                                            Change Profile Picture
                                        </button>
                                    </>
