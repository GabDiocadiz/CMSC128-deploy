...
export default function Notification({ setVisible }) {
    ...
    const notificationViewRef = useRef(null);

    // Close on outside click
    useEffect(() => {
        const handleClickOutside = (event) => {
        const clickedOutsideMain = modalRef.current && !modalRef.current.contains(event.target);
        const clickedInsideView = notificationViewRef.current && notificationViewRef.current.contains(event.target);

        if (clickedOutsideMain && !clickedInsideView && !isNotificationViewVisible) {
            setVisible(false);
        }
    };
    ...
    }, [setVisible, isNotificationViewVisible]);
        ...
        <div ref={modalRef} className="relative bg-white rounded-lg shadow-lg w-[30vw]">
            ...
            <div className="flex items-center justify-between p-4 border-b border-gray-200 rounded-t">
                ...
                <button
                    ...
                    type="button"
                    className="text-gray-400 hover:bg-gray-200 rounded-lg text-sm w-8 h-8 inline-flex justify-center items-center cursor-pointer"
                >
                    <svg
                        className="w-3 h-3"
                        ...
                        {notifications.map((notificationItem) => (
                            ...
                            >
                                <div className="flex-grow">
                                    ...
                                    <p className={`text-xs sm:text-sm mt-1 line-clamp-2 text-left ${notificationItem.status === 'read' ? 'text-gray-500' : 'text-gray-600'}`}>
                                        {notificationItem.announcement?.content || "No content available."}
                                    </p>
                                    <span className="text-gray-400 text-xs group-hover:text-gray-500 mt-1.5 block text-left">
