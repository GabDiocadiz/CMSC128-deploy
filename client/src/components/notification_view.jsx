...
export default function Notification_View({notification_info, setNotification_view}){
    ...
    <div>
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-60 p-4" role="dialog" aria-modal="true" aria-labelledby="notification-view-title">
            <div 
                ref={localRef}
                className="relative bg-white rounded-xl p-6 w-full max-w-2xl max-h-[90vh] shadow-xl flex flex-col animate-fadeIn"
                onClick={(e) => e.stopPropagation()}
            >
                {/* Modal Header */}
                <div className="flex justify-between items-center border-b pb-3 mb-2 mt-2">
                ...
                <div className="flex justify-end pt-4 mt-auto border-t">
                    <button
                        onClick={handleClose}
                        className="bg-emerald-600 text-white px-6 py-2 rounded-lg hover:bg-emerald-700 transition-colors focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-opacity-50 cursor-pointer"
                    >
                        Close
                    </button>
                </div>
